...
import type { LayerType, NodeParams, ModelGraph, TrainStatus, GlobalVariable } from '../types';
import { LAYER_DEFINITIONS } from '../types';
...
export const useDesignerStore = create<DesignerState>((set, get) => ({
  ...
  addNode: (type, position, extra) => {
    ...
    get().addLog('INFO', 'model', `Added ${def.label} node`);
  ...
  addGroup: (label, color, nodeIds) => {
    ...
    // Re-parent selected nodes inside the group
  ...
  loadModel: async (id) => {
    try {
      ...
      const nodes: Node[] = graph.nodes.map((n) => {
        const def = LAYER_DEFINITIONS[n.type as LayerType];
  ...
  loadGraph: (graph) => {
    ...
    set({
      ...
      modelName: graph.meta.name,
      modelDescription: graph.meta.description,
      modelId: null, // Detach from any saved model ID since it's a raw graph load
      selectedNodeId: null,
    });
