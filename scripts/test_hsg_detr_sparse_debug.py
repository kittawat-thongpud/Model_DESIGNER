...
def test_sg_token_contract(torch, device: str) -> None:
    ...
    _require(abs(float(block.gamma.detach().mean()) - 1e-4) < 1e-8, "legacy gamma_init is not 1e-4")
...
def test_amp_forward(torch, device: str) -> None:
    from hsg_detr.nn.sparse_global_token import SGTokenBlock
    from hsg_detr.nn.sparse_global_token_v2 import SGTokenBlockV2
    ...
    for cls in (SGTokenBlock, SGTokenBlockV2):
...
def test_model_forward(torch, device: str, imgsz: int) -> None:
    ...
    _require(all(abs(float(b.gamma.detach().mean()) - 1e-4) < 1e-8 for b in sgb_blocks), "some SGB gamma values are not initialized to 1e-4")
...
def test_v2_model_forward(torch, device: str, imgsz: int) -> None:
    import hsg_detr  # noqa: F401 - registers HSG-DETR modules with Ultralytics
    from ultralytics import RTDETR

    _print_step("HSG-DETR-V2-N instantiate + eval forward")
    cfg = ROOT / "backend" / "hsg_detr" / "configs" / "hsg_detr_v2_n.yaml"
    model = RTDETR(str(cfg)).model.to(device).eval()
    sgb_blocks = [m for m in model.modules() if m.__class__.__name__ == "SGTokenBlockV2"]
    decoders = [m for m in model.modules() if m.__class__.__name__ == "RTDETRDecoderV2"]
    _require(len(sgb_blocks) == 3, f"expected 3 SGTokenBlockV2 modules, got {len(sgb_blocks)}")
    _require(len(decoders) == 1, f"expected 1 RTDETRDecoderV2 module, got {len(decoders)}")
    ...
    tensor_out = out[0] if isinstance(out, (tuple, list)) else out
    _require(torch.is_tensor(tensor_out), f"unexpected V2 model output type: {type(out)!r}")
    _require(torch.isfinite(tensor_out).all().item(), "V2 model forward produced NaN/Inf")
    print(f"PASS v2_blocks={len(sgb_blocks)} output_shape={tuple(tensor_out.shape)}.")
...
def main() -> int:
    ...
    try:
        ...
        if not args.skip_model:
            ...
            test_v2_model_forward(torch, args.device, args.imgsz)
