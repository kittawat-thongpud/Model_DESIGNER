...
def _run_knn_evaluation(job_id: str, root: Path, imagefolder: Path, checkpoint: Path, spec: dict, batch: int, workers: int, out_dir: Path) -> None:
    ...
    model_name = str(job.get("model_name", "")).lower()
    if "detr" in model_name or "detector" in model_name:
        job_storage.append_job_log(job_id, "INFO", "Skipping k-NN evaluation for DINO-DETR (detection) checkpoint - incompatible with DINO self-supervised eval_knn.py")
        return
    ...
    try:
        ...
        for line in proc.stdout:
            ...
            # Expected format: "k-NN eval: 10-NN classifier result: Top1: 100.0, Top5: 100.0"
            if "classifier result:" in line and "Top1:" in line:
                try:
                    # Extract Top1 accuracy
                    top1_str = line.split("Top1:")[1].split(",")[0].strip()
                    knn_accuracy = float(top1_str) / 100.0
